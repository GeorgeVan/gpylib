...
picbkhelpstring="""
第一步：使用 tj 0来统计一下待备份目录的照片的情况。看是否有日期不对、没有日期的情况
"""
...
def _trygeninfofromname(fn):
    ...
    m = re.match(ur'^(\d{4}\-\d\d-\d\d_\d{6})(\([^\(\)]+\))?_?([0-9a-z]*)-?([0-9a-z]*)\.jpg$',fn,re.U|re.I)
    try:
...
def _trygetnote(fn,root,pcache=False, cache={}):
    ...
    def removeall(n, usecache):
        ...
        uselessfns=('CIMG','IMG','IMAG','DSC[ \da-z]{5,}',
                    ...
                    ur'图像',ur'小图',ur'副本',ur'复件',ur'复制',ur'旋转',ur'pic',ur'image',ur'未标题',ur'照片',
                    ur'数码',ur'Olympus',ur'Olyms',ur'iphone',
    ...
    names=fn[:-4].replace(u"/",u" ").replace(u"\\",u" ").split()[len(r):]
...
def printgennote(path,root):
    lastnote=None
    for fn in gpylib.misc.walkfiles(path, lambda x: x.lower().endswith(".jpg"), False):
...
class _PicCensus(object):
    ...
    def adddir(self,path):
        for f in gpylib.misc.walkfiles(path, lambda x: x.lower().endswith(".jpg"), True):
...
class _PicDateTimeCheck(object):
    def __init__(self,root,datebegin,dateend, allowmeta=False, datetimegener=None,
        ...
        for fn in gpylib.misc.walkfiles(root, lambda x: x.lower().endswith(".jpg"), True):
    ...
    def printme(self):
        ...
        print RED+"{} pics cannot be archived.".format(c) if c else GREEN+"All pics can be archived."
...
class _PicGroup(object):
    ...
    def mirror(path, md5dict):
        ...
        for f in os.listdir(path):
            ...
            if os.path.isfile(fullname) and f.lower().endswith(".jpg"):
...
class _PicArchive(object):
    ...
    def adddir(self, path, rootdir):
        gprint("\nAdding [<>] to archive\n", BLUE+BWHITE+path) 
        fns= [f for f in gpylib.misc.walkfiles(path, lambda x: x.lower().endswith(".jpg"), True,"\n\n")]
        gprint(RED+"Randoming the files\n")
        random.shuffle(fns)
        for fn in fns:
        #for fn in gpylib.misc.walkfiles(path, lambda x: x.lower().endswith(".jpg"), True,"\n\n"):
            try:
                self.add(fn, rootdir)
...
def autobakpics(picdir,rootdir,archivedir):
...
def autobakpiccurrentdir(archivedir):
    '''[t bk "I:\PicArchive"]'''
    autobakpics(os.getcwdu(), os.getcwdu(), archivedir)
...
def testarchiveiflost(newdir,archivedir, verbose, del_ifok=False):
    ...
    for fn in gpylib.misc.walkfiles(archivedir, lambda x: x.lower().endswith(".jpg"), True):
        archivedict[os.stat(fn).st_size].append( [fn,None] )
    
    print BLUE+"\nChecking..."
    lostcount, removecount=0,0
    
    for fn in gpylib.misc.walkfiles(newdir, lambda x: x.lower().endswith(".jpg"), True,pre="\n"):
        gpylib.misc.printworking()
...
def sonhhdr_genlong(isFake=True):
    """遍历当前目录，将所有文件的文件名name更改为 YYYY_MM_DD-name
    ...
    for f in os.listdir(r):
        t = time.strftime("%Y_%m%d-", time.localtime(os.stat(f).st_mtime));
        if f.find(t) != -1:
            break

        f2 = t+f
        if isFake:
            print u"#ren {} {}".format(f,f2)
        else:
            print u"ren {} {}".format(f,f2)
            os.rename(f, f2)

def sonhhdr_genbak(isFake=True):
