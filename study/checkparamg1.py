...
def validParam(*condvalues, **condkwvalues):
    ...
    def vpinner(func):
        ...
        def wrapper(*values, **kwvalues):
            '''doc of wrapper'''
            #调用是真正参数字典
            dctCallArgs = _getcallargs(func,values, kwvalues)
            
            name, validation = None, None
            def verify(v):
                if not validation(v):
...
def _assignNames(names,varsname,kwsname,values,kwvalues):
    '''通用的对齐程序
    names：有名参数列表 list/tuple
    varsname: 无名参数的名称 string/None
    kwsname: 关键词参数的名称 string/None
    
    values: 调用的时候的无名值列表 list/tuple
    kwvalues: 调用时的有名值列表 dict
    
    定义时的validParam，调用此函数时，varname和kwname是空
    '''

    _dctArgs = {}
    
    #第一步：把values的前 len(names)个，和names一一对应
    if names and values:
        _dctArgs.update(zip(names,values))
    
    #第二步：将values剩下,都和varsname对应
    unusedvarvalues=None
    if values:
        unusedvarvalues = values[len(_dctArgs):]
    if varsname and unusedvarvalues:
        _dctArgs[varsname] = unusedvarvalues
        unusedvarvalues = None
    
    #第三步：kwvalues里面的名字，如果在names里面有，则覆盖；否则，都和kwsname对应
...
def _getdefargs(func, condvalues, condkwvalues):
    ...
    #print "\n\n".join([ "%d:\n"%-i + "\n".join(["\t"+str(item) for item in s[1:]]) for i,s in enumerate(inspect.stack()) ])
    ...
    _dctArgs,unusedvarvalues,repeatekeys, unusedkeys = _assignNames(names,None,None,condfuncs,condkwfuncs)
...
def _getcallargs(func, values, kwvalues):
    '''获取定义时的各参数名-判断函数的字典
    '''
    names, varsname, kwsname = inspect.getargspec(func)[:3]
                
    _dctArgs,unusedvarvalues,repeatekeys,unusedkeys = _assignNames(names,varsname,kwsname,values,kwvalues)
    
    if unusedvarvalues or repeatekeys or unusedkeys:
        print "*************************************************"
...
def _functest(func, isCkPass, *argnames, **kws):
    ...
    print "\t{0}({1}{2}{3}) {4}\n\t\tTestng".format(func.__name__, 
                ...
                ", " if len(argnames)>0 and len(kws)>0 else "",
